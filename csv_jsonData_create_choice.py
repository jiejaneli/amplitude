...
def func_25_device_manufacturer():
...
def func_30_location_lat():
  locationLatitudes = ["37.322998", "37.338208", "40.712775", "34.520381", "39.904200", "38.990254"] 
  if func_true_false() == True:
     return locationLatitudes[rannum(0, len(locationLatitudes) - 1)]
  else:
     return empty_string


# function to generate field 31 - location_longitude (string)  (No 31 field for event)
def func_31_location_lng():
   locationLongitudes = ["-122.032182", "-121.886329", "-74.005973", "134.878664", "116.407396", "-77.124945"] 
   if func_true_false() == True:
      return locationLongitudes[rannum(0, len(locationLongitudes) - 1)]
   else:
      return empty_string
...
def func_37_revenue():
  revenues = ["5.88", "100.50", "30.60", "150.68", "300.55", "1000,80", "28.90", "120.40"]
  if func_true_false() == True:
     return revenues[rannum(0, len(revenues) - 1)]
  else:
     return empty_string


# function to generate field 38 - ip_address (string) (No 38 field for event)
def func_38_ip_address():
  ipAddresses = ["127.0.0.1", "192.168.10.1", "192.168.20.1", "10,168.172.55", "136.146.166.33", "122.34.12.1"]
  if func_true_false() == True:
     return ipAddresses[rannum(0, len(ipAddresses) - 1)]
  else:
     return empty_string
...
def main():
  ...
  # define CSV file separator (normally ",")
  CSV_delimiter = ";"
  ...
  choices = [0,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3]

  # next generate ne events and write them onto output file
  for i in range(ne):
    myEvent = ""

    #choices[47] = [0,1,2,3,2,1,3,1,2,3,2,1,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3]
    # 1) add 'app'
    myApp = func_01_app()
    ...
    # 18) add 'library'
    myItem = func_18_library()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 19) add 'processed_time'
    myItem = func_19_processed_time()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 20) add 'user_creation_time'
    myItem = func_20_user_creation_time()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 21) add 'platform'
    myItem = func_21_platform()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 22) 'add os_name'
    myItem = func_22_os_name()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 23) add 'os_version'
    myItem = func_23_os_version()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 24) add 'device_brand'
    myItem = func_24_device_brand()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 25) add 'device_manufacturer'
    myItem = func_25_device_manufacturer()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 26) add 'device_model'
    myItem = func_26_device_model()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 27) add device_carrier
    myItem = func_27_device_carrier()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 28) add 'device_type'
    myItem = func_28_device_type()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 29) add 'device_family'
    myItem = func_29_device_family()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 30) add 'location_lat'
    myItem = func_30_location_lat()
    myEvent = myEvent + str(myItem) + CSV_delimiter


    # 31) add 'location longitude' 31
    myItem = func_31_location_lng()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 32) add 'country'
    myItem = func_32_country()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 33) add 'language'
    myItem = func_33_language()
    myEvent = myEvent + str(myItem) + CSV_delimiter
    ...
    # 37) add 'revenue'
    myItem = func_37_revenue()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 38) add 'ip_address'
    myItem = func_38_ip_address()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 39) add 'paying'
    myItem = func_39_paying()
    myEvent = myEvent + str(myItem) + CSV_delimiter

    # 40) add 'start_version'
    myItem = func_40_start_version()
    myEvent = myEvent + str(myItem) + CSV_delimiter
