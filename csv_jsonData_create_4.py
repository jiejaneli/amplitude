...
def main():
  ...
  for i in range(ne):
    ...
    # 1) add 'app'
    myApp = func_01_app()
    myEvent = myEvent + str(myApp) + csvSeparator

    # 2) add 'amplitude_id'
    myAmplitude_id = func_02_amplitude_id()
    myEvent = myEvent + str(myAmplitude_id) + csvSeparator

    # 3) add 'user_id'
    myUser_id = func_03_user_id()
    myEvent = myEvent + myUser_id + csvSeparator
    

    # 4) add 'device_id'
    myDevice_id = func_04_device_id()
    myEvent = myEvent + myDevice_id + csvSeparator

    # 5) add 'event_time'
    startime = "2017-11-01 10:15:34"
    endtime =  "2017-12-31 12:27:54"
    myDateTime = func_05_08_dateTime_random(startime, endtime)
    myEvent = myEvent + str(myDateTime) + csvSeparator
    ...
    myEvent = myEvent + str(myEvent_id) + csvSeparator

    # 10) add 'session_id' 
    mySession_id = func_10_session_id()
    myEvent = myEvent + str(mySession_id) + csvSeparator

    # 11) add 'event_type'
    myEvent_type = func_11_event_type()
    myEvent = myEvent + str(myEvent_type) + csvSeparator


    # 12) add 'amplitude_event_type'
    myItem = func_12_amplitude_event_type()
    myEvent = myEvent + str(myItem) + csvSeparator
